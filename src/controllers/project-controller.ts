...
export const getProjects = async (req: Request, res: Response) => {
	try {
		// get all projects of current user
		const projects = await ProjectModel.find({ createdBy: req.user._id }).sort({ createdAt: -1 });
		res.json({ projects });
	} catch (error) {
		console.log(error);
...
export const updateProject = async (req: Request, res: Response) => {
	...
	try {
		...
		// update the project
		project = await ProjectModel.findByIdAndUpdate({ _id: id }, { $set: newProject }, { new: true });
		res.json(project);
	} catch (error) {
		console.log(error);
...
export const deleteProject = async (req: Request, res: Response) => {
	...
	try {
		...
		await TaskModel.deleteMany({ projectId: new Types.ObjectId(id as string) });

		// delete the project
		await ProjectModel.findOneAndDelete({ _id: new Types.ObjectId(id as string) });
